...
SQL_FIX_FUNCTION = """
CREATE OR REPLACE FUNCTION solde_fin_pour_exercice(p_exercice_id bigint)
RETURNS TABLE(
...
SQL_DROP_OLD_FUNCTION = "DROP FUNCTION IF EXISTS solde_fin_pour_exercice(integer);"
SQL_DROP_NEW_FUNCTION = "DROP FUNCTION IF EXISTS solde_fin_pour_exercice(bigint);"


class Migration(migrations.Migration):

    dependencies = [
        ('facture', '0023_create_solde_fin_pour_exercice_function'),
    ]

    operations = [
        migrations.RunSQL(
            sql=SQL_DROP_OLD_FUNCTION,
            reverse_sql=migrations.RunSQL.noop,
        ),
        migrations.RunSQL(
            sql=SQL_FIX_FUNCTION,
            reverse_sql=SQL_DROP_NEW_FUNCTION,
        ),
    ]